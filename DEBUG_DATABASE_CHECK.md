...
```sql
-- Check if wallet_kyc table exists and has data
SELECT 
    table_name,
    column_name,
    data_type
...
-- Check if there's any wallet_kyc data for your user
SELECT 
    id,
    user_id,
    status,
...
-- Check profiles table for customer_id
SELECT 
    user_id,
    customer_id,
    first_name,
...
-- Check wallets table for is_active status
SELECT 
    user_id,
    is_active,
    balance,
